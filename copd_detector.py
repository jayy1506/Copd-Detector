...
def predict_copd(image_array):
    ...
    try:
        ...
        # Make prediction directly without dummy initialization
...
def grad_cam(model, img_array, layer_name):
    ...
    try:
        ...
        # Get the ResNet model (first layer)
        resnet_model = model.layers[0]
        
        # Get the specific layer from ResNet
        target_layer = resnet_model.get_layer(layer_name)
        
        # Build the gradient model correctly using the main model
        ...
        with tf.GradientTape() as tape:
            ...
            # We want to maximize the class score for better visualization
        ...
        # Apply ReLU to only show positive influences
...
def visualize_grad_cam(img, heatmap, save_path, alpha=0.5):
    """Visualize Grad-CAM results and save to file"""
    try:
        # Resize heatmap to match image size
        heatmap = cv2.resize(heatmap, (img.shape[1], img.shape[0]))
        
        # Convert heatmap to RGB with enhanced visibility
        heatmap = np.uint8(255 * heatmap)
        heatmap = cv2.applyColorMap(heatmap, cv2.COLORMAP_JET)
        
        # Enhance the heatmap for better visibility
        # Increase the intensity of the heatmap
        heatmap = cv2.convertScaleAbs(heatmap, alpha=1.2, beta=10)
        
        # Superimpose heatmap on original image with better blending
        superimposed_img = cv2.addWeighted(img, alpha, heatmap, 1-alpha, 0)
        
        # Add contour highlighting for clearer boundaries
        gray_heatmap = cv2.cvtColor(heatmap, cv2.COLOR_RGB2GRAY)
        _, thresh = cv2.threshold(gray_heatmap, 50, 255, cv2.THRESH_BINARY)
        contours, _ = cv2.findContours(thresh, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        
        # Draw contours on the superimposed image for emphasis
        cv2.drawContours(superimposed_img, contours, -1, (255, 255, 0), 2)
        ...
        # Fallback: save original image with basic heatmap
        try:
            heatmap = cv2.resize(heatmap, (img.shape[1], img.shape[0]))
            heatmap = np.uint8(255 * heatmap)
            heatmap = cv2.applyColorMap(heatmap, cv2.COLORMAP_JET)
...
def find_last_conv_layer(model):
    ...
    try:
        ...
        # Preferred layers that work well for Grad-CAM
        # These have been tested and work reliably
        preferred_layers = [
            'conv5_block3_2_conv',
            'conv5_block2_2_conv',
            'conv4_block6_2_conv',
        ...
        # Check if any preferred layers exist
        for layer_name in preferred_layers:
            try:
                # Try to access the layer
                resnet_model.get_layer(layer_name)
                print(f"Found preferred conv layer: {layer_name}")
                return layer_name
            except:
                continue
        
        # Fallback: try to find convolutional layers by name pattern
        conv_layers = []
        for layer in resnet_model.layers:
            # Look for layers with 'conv' in their name
            if 'conv' in layer.name.lower() and 'conv' in layer.name and layer.name.endswith('conv'):
                conv_layers.append(layer.name)
        
        # Prefer middle to later convolutional layers for better feature representation
        # These typically provide better localization than very early or very late layers
        preferred_patterns = ['block3', 'block4', 'block5', 'block6']
        
        # Try to find a preferred layer first
        for pattern in preferred_patterns:
            for layer_name in reversed(conv_layers):  # Check from later layers backward
                if pattern in layer_name and layer_name.endswith('conv'):
                    print(f"Found preferred conv layer: {layer_name}")
                    return layer_name
        
        # If no preferred layer found, return the last suitable conv layer
        if conv_layers:
            last_conv = conv_layers[-1]
            print(f"Found last conv layer: {last_conv}")
            return last_conv
...
def analyze():
    ...
    try:
        ...
        # Generate result HTML without Grad-CAM (as per requirements)
        result_html = f'''
        <!DOCTYPE html>
        <html>
        <head>
        ...
        <body>
            <div class="container">
                ...
                <div class="clinical-findings">
                    <h3 style="color: #333; text-align: center; margin-bottom: 20px;">📋 Clinical Assessment</h3>
                    
                    <div class="finding-item">
                        <div class="finding-label">Presence of COPD</div>
                        <div>{assessment['result_class']}</div>
                    </div>
                    
                    <div class="finding-item">
                        <div class="finding-label">Severity Level</div>
                        <div>{assessment['severity_level']}</div>
                    </div>
                    
                    <div class="finding-item">
                        <div class="finding-label">Medication Required</div>
                        <div>{assessment['medication_needed']}</div>
                    </div>
                    
                    <div class="finding-item">
                        <div class="finding-label">Clinical Recommendations</div>
